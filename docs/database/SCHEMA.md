...
this project's compiled entity classes under the PostgreSQL dialect: **84 tables, 617 columns**, 84
...
**Primary keys and id generation.** Every one of the 84 tables has a primary key. Entity tables use
...
84 tables. Names in **bold** are the main table of a module; the rest are side tables, join tables
and logs.
...
| [`writing_book_genres`](#writing_book_genres) | Publishment | Multiple enum-backed genres per book. |
...
`writing_book_genres` holds the enum-backed genres — three overlapping notions of "genre" survive
in the schema. `series_id` is auto-generated in `@PrePersist` as `"series-" +
...
Genres a book belongs to — a book may have several. `@ElementCollection` of the `BookGenre` enum.
...
the literal strings `POLITICAL`, `ACADEMIC` or `ESSAY`. Queries filtering on `POLITICS` will miss
those rows. `findAllByGenre` uses `:genre MEMBER OF w.bookGenres`, which joins this table with no
index on `book_genre`.
...
These thirteen column names are the complete set of check constraints in the generated DDL.
...
| `writing_book_genres.book_genre` | `BookGenre` | `varchar(30)` | `POETRY`, `NOVEL`, `SHORT_STORY`, `DRAMA`, `HISTORY`, `BIOGRAPHY`, `PHILOSOPHY`, `RELIGION`, `FOLKLORE`, `POLITICS`, `SOCIOLOGY`, `ECONOMICS`, `LAW`, `LINGUISTICS`, `ARTS`, `CULTURAL`, `SCIENCE`, `MEDICINE`, `EDUCATIONAL`, `CHILDREN`, `TRAVEL`, `OTHER`, `ESSAY`, `POLITICAL`, `ACADEMIC` |
